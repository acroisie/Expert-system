...
func (rule Rule) Solving(facts []Fact, display bool) (Value, Value, error) {
	...
	if err != nil {
		return UNDETERMINED, UNDETERMINED, err
	}
	...
	if display {
		fmt.Println(fmt.Sprintf("Rule solving LeftValue: %s, RightValue: %s", leftValue, rightValue))
	}
...
func SolveRules(rules []Rule, facts []Fact) {
	...
	var lap int = 0
	for change > 0 && lap < 30 {
		fmt.Println(fmt.Sprintf("---------- LAP %d ----------", lap))
		change = 0
...
func (rule Rule) ExploitResults(leftValue Value, rightValue Value, facts []Fact, display bool) (int, error) {
	if leftValue.real() && rightValue == INIT_FALSE {
		if rule.RightVariable != nil {
			err := SetFactValueByLetter(facts, rule.RightVariable.Letter, leftValue, true)
			return 1, err
		} else {
			return rule.RightExpressionGroup.resultDeduction(leftValue, 0, facts)
		}
	} else if leftValue == INIT_FALSE && rightValue.real() {
		if rule.LeftVariable != nil {
			err := SetFactValueByLetter(facts, rule.LeftVariable.Letter, rightValue, true)
			return 1, err
		} else {
